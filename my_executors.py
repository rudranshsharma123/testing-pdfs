...
class TextCrafter(Executor):
    ...
    def filter(self, docs: DocumentArray, **kwargs):
        filtered_docs = DocumentArray(
            d for d in docs if d.mime_type == 'text/plain'
        ) 
        print(filtered_docs)
        return filtered_docs
...
class ImageCrafter(Executor):
    ...
    def craft(self, docs: DocumentArray, fn):
        ...
        filtered_docs = DocumentArray(
            d for d in docs.traverse_flat(['c']) if d.mime_type == 'image/*'
        ) if fn == "convert_image_uri_to_blob" else DocumentArray(d for d in docs if d.mime_type == 'image/*')
        print(filtered_docs)
        for doc in filtered_docs:
            # getattr(doc, fn)()
            if fn == "convert_image_uri_to_blob":
                doc.convert_image_blob_to_uri(height=100, width=100)
